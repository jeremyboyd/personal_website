...
Data were modeled using Bayesian hierarchical logistic regression,[^16]^,^[^17]^,^[^18] with coder and intersection treated as random effects with the maximal random effects structure.[^19] For simplicity of presentation the traffic variable was recoded into the categories *light* and *heavy*, with light referring to traffic values of one or two, and heavy referring to values greater than two. Two separate models were fit. The *full model* predicted cutting as a function of vehicle value, traffic, and their interaction. Additional covariates included driver sex, driver age, and the time of day. The *light model* was identical, except that only data from the light traffic condition were used, and traffic was dropped as a predictor. The purpose of the light model was to assess whether there was a reliable effect of vehicle status on cutting when the attentional load on drivers was low.
...
Nineteen of the 778 drivers, accounting for 2.4% of the data, were excluded from analysis, either because they were not visible enough to record their sex and age, or because it was not possible to definitively determine whether a cut had occurred (e.g., due to pedestrians or cyclists disrupting the queue). This left us with analyzable data from 759 drivers.

For the full model the overall data pattern is illustrated below in Figure 5. Drivers of higher status vehicles cut more than drivers of lower status vehicles, *B* = `r fixed_effects %>% filter(effect == "c_vehicle_status") %>% pull(Estimate) %>% format(digits = 1, nsmall = 2)`, 95% CrI [`r fixed_effects %>% filter(effect == "c_vehicle_status") %>% pull(Q2.5) %>% format(digits = 1, nsmall = 2)`, `r fixed_effects %>% filter(effect == "c_vehicle_status") %>% pull(Q97.5) %>% format(digits = 1, nsmall = 2)`]. Drivers cut more under heavy traffic conditions, *B* = `r fixed_effects %>% filter(effect == "c_traffic_heavy") %>% pull(Estimate) %>% format(digits = 1, nsmall = 2)`, 95% CrI [`r fixed_effects %>% filter(effect == "c_traffic_heavy") %>% pull(Q2.5) %>% format(digits = 1, nsmall = 2)`, `r fixed_effects %>% filter(effect == "c_traffic_heavy") %>% pull(Q97.5) %>% format(digits = 1, nsmall = 2)`]. And while the interaction of vehicle status and traffic was not credibly different from zero at the 95% level, *B* = `r fixed_effects %>% filter(effect == "c_vehicle_status:c_traffic_heavy") %>% pull(Estimate) %>% format(digits = 1, nsmall = 2)`, [`r fixed_effects %>% filter(effect == "c_vehicle_status:c_traffic_heavy") %>% pull(Q2.5) %>% format(digits = 1, nsmall = 2)`, `r fixed_effects %>% filter(effect == "c_vehicle_status:c_traffic_heavy") %>% pull(Q97.5) %>% format(digits = 1, nsmall = 2)`], it was at the 88% level, [`r inter_88[1, 1] %>% format(digits = 1, nsmall = 2)`, `r inter_88[1, 2] %>% format(digits = 1, nsmall = 2)`]. All other effects in the full model were null.
...
# Fit a single model--full.
# Then in results, in order to investigate the interaction we do a couple of things: (1) assess the interaction at lower CrIs, and (2) fit separate light and heavy models, which will show that there's a reliable vehicle status effect in the light but not the heavy condition.
