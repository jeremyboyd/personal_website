...
Thirty-five undergraduate students at the University of California, San Diego were trained to code vehicle values using the same five-point scale found in Piff et al.: 1 = cars worth $3,000 or less; 2 = $3001-$10,000; 3 = $10,001-$25,000; 4 = $25,001-$40,000; 5 = cars worth more than $40,000.

Fix formatting around prices above...

### Procedure

Coders positioned themselves unobtrusively at all-way stop intersections. For example, they sat at bus stops, in parked cars, or feigned phone coversations. Coders pseudo-randomly selected approaching vehicles for coding, witht he constraint that vehicles approaching empty intersections were not selected, and commercial vehicles were not selected. Prior to stopping, the estimated value of each selected vehicle was recorded using the vehicle value scale described above, and the amount of traffic was measured--i.e., coders noted the number of lanes in the intersection in which other cars were already queued (see Figure 4). Coders then observed whether drivers waited their turn before proceeding through the intersection, or whether they jumped the queue and cut off other drivers, in violation of California Vehicle Code. As drivers pulled away, their perceived sex and age (1 = 16-35 years old; 2 = 36-55; 3 = 56 and older) were recorded, as well as the time of day.
...
Nineteen of the 778 drivers, accounting for 2.4% of the data, were excluded from analysis, either because they were not visible enough to record their sex and age, or because it was not possible to definitively determine whether or not a cut had occurred (e.g., due to pedestrians or cyclists disrupting the queue).

Data from the the remaining 759 drivers were modeled using Bayesian hierarchical logistic regression (cite R, Stan, brms). For simplicity of presentation, traffic values of one to two were recoded as *light*, and values greater than two were recoded as *heavy*. Two separate models were fit. In the *full model*, the outcome was whether a cut had occurred, and the main predictors of interest were vehicle value, traffic, and their interaction. Covariates included driver sex, driver age, and the time of day. The clustering of data points by coder and intersection was taken into account by modeling coder and intersection as random effects with the maximal random effects structure.[^15] The *light model* was identical, except that only data from the light traffic condition were modeled.

Results are shown below in Figure 5.

```{r results, echo = FALSE, dev = "svg", out.width = "700", fig.cap = "Probability of cutting as a function of vehicle status and traffic. Lines represent predicted means. Ligher and darker shading represents the 95% and 80% credible intervals, respectively."}
...
[^15]: Barr, D.J., Levy, R., Scheepers, C., & Tily, H.J. (2013). Random effects structure for confirmatory hypothesis testing: Keep it maximal. *Journal of Memory and Language*, 68, 255-278.
